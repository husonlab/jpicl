...
int main(int argc, char *argv[]) {
  ...
  if (random_tree==0) { // read in user species tree
	...
	if (tf==NULL) { printf("File treefile not found. Exiting.\n\n"); exit(1);}
  ...
  if (anneal==0) {
	...
	// re-order ppTwoRow and write tree to outtree
        int temptax;
        for (i=0; i<ntaxa; i++) {
	...
	fprintf(out,"Current tree in coalescent units: \n");
        ...
        fprintf(out,";\n\n");
        for (i=1;i<ntaxa; i++) TimeVec[ntaxa+i] = TimeVec[ntaxa+i]*theta;
	printf("Tree has been written to file outtree\n\n");
  ...
  // if anneal = 2
	...
	fclose(out);
	printf("Results have been written to file outtree -- exiting.\n\n");
	//exit(1);
